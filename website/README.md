...
См. файлы:
- `PRISMA_FIX.md` - подробные решения проблемы с Prisma
- `fix-prisma.ps1` - скрипт для автоматического решения
- `INSTALL.md` - общие инструкции по решению проблем
...
Или используйте Docker (самый надежный способ):
```powershell
docker-compose up -d
```
...
**Подробная документация:** См. `INTEGRATION_SETUP.md`
